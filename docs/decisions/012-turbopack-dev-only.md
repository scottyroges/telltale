# ADR 012: Turbopack for Development Only

**Status:** Accepted
**Date:** 2025-02

## Context
Next.js supports Turbopack (Vercel's Rust-based bundler) for both `next dev` and `next build`. We need to decide whether to use it for production builds as we deploy to Vercel (Plan 0.6).
